...
namespace SpringHeroBank.model
{
    ...
    {   
        public Boolean Save(Account account)
        {
            ...
            var sqlQuery = "insert into `spring` " +
        ...
        public Boolean CheckTransfer(Account account1, Account account2, Transaction historyTransaction)
        {
            ...
            // Mở transaction, block try - catch
            var transaction = DbConnection.Instance().Connection.BeginTransaction();
            try
            {

                //Lấy thông tin tài khoản một lần nữa đảm bảo mới nhất
                var queryBalance = "select balance from `spring` where username = @username and status = @status";
                ...
                else
                {
                    ...
                    var queryUpdateAccountBalance =
                        "update `spring` set balance = @balance where username = @username and status = 1";
                ...
                //Lấy thông tin tài khoản nhận, đảm bảo tài khoản không bị khoá hoặc inactive.
        ...
        {
            DbConnection.Instance().OpenConnection();
            var queryString = "select * from  `spring` where username = @username and status = 1";
